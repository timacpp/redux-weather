...
export const calculateWinner = (board, boardSize) => generateWinningLines(boardSize).reduce(
    (currentWinner, line) => {
...
const generateWinningLines = (boardSize) => {
    ...
    const winningLines = winningLineRange.map(
        (i) => [
    ...
    winningLines.push(winningLineRange.map((i) => ({ x: i, y: boardSize - 1 - i })));
